...
def setup_profile_mock(mock_supabase, timezone='UTC'):
    """Setup the profile fetch mock chain."""
    mock_execute = create_mock_execute_response({'timezone': timezone})
    mock_supabase.table.return_value.select.return_value.eq.return_value.single\
        .return_value.execute.return_value = mock_execute
...
class TestAdventureProcessorRoundProcessing:
    ...
    def test_returns_zero_for_non_active_adventure(self, mock_supabase_base, sample_adventure):
        """Test that non-active adventures return 0 rounds processed."""
        sample_adventure['status'] = 'completed'

        result = process_adventure_rounds(sample_adventure)

        assert result == 0
        # Should not call any RPC functions
        mock_supabase_base.rpc.assert_not_called()

    def test_skips_processing_when_on_break(self, mock_supabase_base, sample_adventure):
    ...
    def test_clears_expired_break_status(self, mock_supabase_base, sample_adventure):
        """Test that break status is cleared when break end date has passed."""
        today = date.today()
        yesterday = today - timedelta(days=1)

        sample_adventure['is_on_break'] = True
        sample_adventure['break_end_date'] = yesterday.isoformat()
        ...
        mock_supabase_base.rpc.return_value.execute.return_value = \
            create_mock_execute_response([{'damage': 100, 'new_hp': 200}])
    ...
    def test_clears_break_status_with_no_end_date(self, mock_supabase_base, sample_adventure):
    ...
    def test_processes_rounds_successfully(self, mock_supabase_base, sample_adventure):
    ...
    def test_handles_rpc_none_response(self, mock_supabase_base, sample_adventure):
        """Test handling of None response from RPC."""
        setup_profile_mock(mock_supabase_base)

        # Mock RPC returning None
        mock_supabase_base.rpc.return_value.execute.return_value = \
            create_mock_execute_response(None)
    ...
    def test_handles_rpc_dict_response(self, mock_supabase_base, sample_adventure):
        """Test handling of dict (not list) response from RPC."""
        setup_profile_mock(mock_supabase_base)

        # Mock RPC returning dict instead of list
        mock_supabase_base.rpc.return_value.execute.return_value = \
            create_mock_execute_response({'damage': 80, 'new_hp': 220})
    ...
    def test_completes_adventure_on_victory(self, mock_supabase_base, sample_adventure):
        """Test that adventure is completed when monster HP reaches 0."""
        setup_profile_mock(mock_supabase_base)

        # Mock calculate_adventure_round RPC
        mock_supabase_base.rpc.return_value.execute.side_effect = [
    ...
    def test_completes_adventure_on_deadline_passed(self, mock_supabase_base, sample_adventure):
    ...
    def test_skips_completion_if_already_completed(self, mock_supabase_base, sample_adventure):
    ...
    def test_handles_exception_during_round_processing(self, mock_supabase_base, sample_adventure):
        """Test that exceptions during round processing are handled gracefully."""
        setup_profile_mock(mock_supabase_base)

        # Mock RPC that raises exception
        mock_supabase_base.rpc.return_value.execute.side_effect = Exception("Database error")
...
class TestCompleteAdventure:
    ...
    def test_completes_adventure_successfully(self, mock_supabase_base):
        """Test successful adventure completion."""
        # Mock RPC response
        mock_supabase_base.rpc.return_value.execute.return_value = \
            create_mock_execute_response([{'is_victory': True, 'xp_earned': 450, 'status': 'completed'}])

        result = complete_adventure('adv-123')
    ...
    def test_handles_dict_response(self, mock_supabase_base):
        """Test handling of dict (not list) response from RPC."""
        # Mock RPC response as dict
        mock_supabase_base.rpc.return_value.execute.return_value = \
            create_mock_execute_response({'is_victory': False, 'xp_earned': 200, 'status': 'completed'})
    ...
    def test_returns_none_on_rpc_none_response(self, mock_supabase_base):
    ...
    def test_returns_none_on_rpc_exception(self, mock_supabase_base):
        """Test that None is returned when RPC raises exception."""
        mock_supabase_base.rpc.return_value.execute.side_effect = Exception("RPC failed")

        result = complete_adventure('adv-123')

        assert result is None

    def test_handles_dict_with_none_data(self, mock_supabase_base):
        """Test handling of list response with None inner data."""
        mock_supabase_base.rpc.return_value.execute.return_value = \
            create_mock_execute_response([None])  # List with None element
...
class TestAdventureProcessorIntegration:
    ...
    def test_full_round_processing_workflow(self, mock_supabase_base):
        ...
        mock_supabase_base.table.return_value.select.return_value.eq.return_value.single\
            .return_value.execute.side_effect = dynamic_execute_response
        ...
        mock_supabase_base.rpc.return_value.execute.side_effect = dynamic_rpc_response

        result = process_adventure_rounds(adventure)
